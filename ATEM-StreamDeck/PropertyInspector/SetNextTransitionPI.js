// Set Next Transition Action Property Inspector using easypi-v2

var settingsCache = {};

// Initialize PropertyInspector after WebSocket connection
function initPropertyInspector() {
    console.log('Set Next Transition PI initialized');
...
function loadConfiguration(payload) {
    console.log('Loading Set Next Transition configuration:', payload);
    ...
    if (settingsCache.transitionStyle === undefined) settingsCache.transitionStyle = 0;
    if (settingsCache.transitionDuration === undefined) settingsCache.transitionDuration = 1.0;
...
function updateFormElements(settings) {
    ...
    const transitionStyleElement = document.getElementById('transitionStyle');
    if (transitionStyleElement) {
        transitionStyleElement.value = settings.transitionStyle !== undefined ? settings.transitionStyle : 0;
    }
    
    // Update Transition Duration
    const transitionDurationElement = document.getElementById('transitionDuration');
    if (transitionDurationElement) {
        transitionDurationElement.value = settings.transitionDuration !== undefined ? settings.transitionDuration : 1.0;
    }
...
function setSettings() {
    ...
    Array.prototype.forEach.call(elements, function (elem) {
        ...
        if (elem.type === 'checkbox') {
            ...
            if (key === 'mixEffectBlock' || key === 'transitionStyle') {
                payload[key] = parseInt(value) || 0;
            } else if (key === 'transitionDuration') {
                payload[key] = parseFloat(value) || 1.0;
            } else {
