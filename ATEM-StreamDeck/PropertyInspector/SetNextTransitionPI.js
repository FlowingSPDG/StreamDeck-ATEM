...
function onATEMInfoReceived(payload) {
    ...
    if (payload && payload.ipAddress) {
        ...
        if (currentIP === payload.ipAddress) {
            ...
            updateTransitionStyleOptions(payload.transitionStyles || []);
        }
    }
}
...
// Update Transition Style options
function updateTransitionStyleOptions(transitionStyles) {
    const transitionStyleElement = document.getElementById('transitionStyle');
    if (!transitionStyleElement || transitionStyles.length === 0) return;
    
    const currentValue = transitionStyleElement.value;
    
    // Clear existing options
    transitionStyleElement.innerHTML = '';
    
    // Add options based on available transition styles
    transitionStyles.forEach(style => {
        const option = document.createElement('option');
        option.value = style.value;
        option.textContent = style.name;
        transitionStyleElement.appendChild(option);
    });
    
    // Restore selection if still valid
    const validStyle = transitionStyles.find(style => style.value == currentValue);
    if (validStyle) {
        transitionStyleElement.value = currentValue;
    } else if (transitionStyles.length > 0) {
        transitionStyleElement.value = transitionStyles[0].value;
        // Trigger settings update
        setSettings();
    }
}