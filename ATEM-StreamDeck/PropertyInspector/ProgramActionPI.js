// Program Action Property Inspector using easypi-v2

var settingsCache = {};

// Initialize PropertyInspector after WebSocket connection
function initPropertyInspector() {
    console.log('Program Action PI initialized');
...
function loadConfiguration(payload) {
    console.log('Loading Program Action configuration:', payload);
    ...
    if (settingsCache.inputId === undefined) settingsCache.inputId = 1;
    if (settingsCache.showTally === undefined) settingsCache.showTally = true;
...
function updateFormElements(settings) {
    ...
    if (inputIdElement) {
        inputIdElement.value = settings.inputId !== undefined ? settings.inputId : 1;
    }
    
    // Update Show Tally
    const showTallyElement = document.getElementById('showTally');
    if (showTallyElement) {
        showTallyElement.checked = settings.showTally !== undefined ? settings.showTally : true;
    }
...
function setSettings() {
    ...
    Array.prototype.forEach.call(elements, function (elem) {
        ...
        if (elem.type === 'checkbox') {
            ...
            if (key === 'mixEffectBlock' || key === 'inputId') {
